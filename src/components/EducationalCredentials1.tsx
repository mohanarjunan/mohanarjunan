...
export const EducationalCredentials1 = () => {
  const [odd, setOdd] = useState<number>(
    workExperiences.length % 2 == 0 ? 0 : 1
  );
      ...
      <div id="educational-credentials" className="bg-surface-200 p-5 h-fit pb-20 flex flex-col items-center justify-center">
        ...
        <div className="md:w-3/5 w-full flex flex-col md:space-y-5 space-y-10 py-10 text-sm relative">
          ...
          {educationalCredentials.map((education: any, index: number) => (
            ...
            <div
              key={index}
              className={`w-full flex md:flex-row flex-col-reverse md:gap-20 gap-4 md:${
                index % 2 == odd ? "" : "flex-row-reverse"
              } md:px-8 ps-8 relative`}
            >
              {/* Dot */}
              <div
                className={`hidden md:block absolute w-5 h-5 overflow-hidden bg-surface-200 rounded-full top-1/2 transform -translate-y-1/2 left-1/2 -translate-x-1/2 p-1.5`}
              >
              ...
              >
                {/* Dot */}
                <div
                  className={`md:hidden absolute w-5 h-5 overflow-hidden bg-surface-200 rounded-full top-1/2 transform -translate-y-1/2 -left-11 p-1.5`}
                >
                  <div className={`w-full h-full bg-brand-100 rounded-full`} />
                </div>
                ...
                {/* Current Hover */}
                <div
                  className={`absolute w-5 h-5 overflow-hidden bg-surface-300 top-1/2 transform -translate-y-1/2 border-4 border-transparent z-0 group-hover:border-dark-500 group-hover:bg-brand-100 rotate-45 ${
                    index % 2 == odd ? "-right-3" : "-left-3"
                  } ${index == odd ? "p-1" : "p-1.5 "}`}
                />
              ...
              {/* Company Block */}
              <div
                className={`relative md:w-1/2 flex flex-col justify-center items-start ps-1 md:ps-0 md:${
                  index % 2 == odd ? "items-start -ps-10" : "items-end"
                } gap-0.5`}
              >
