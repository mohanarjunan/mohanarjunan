...
export const Comments = () => {
  ...
  return (
    <div className="flex justify-center bg-surface-200 py-10 text-light-200">
      <div
        ref={containerRef}
        ...
        {comments.map((comment: any, index: number) => (
          ...
          >
            ...
            <div className='flex flex-col self-end mt-3'>
              <div className='text-lg text-dark-200 font-serif italic flex self-end me-2'>- {comment.author}</div>
              <span className='w-fit text-xs bg-surface-200 text-dark-200 py-1 px-4 rounded-full'>{comment.role} @ {comment.at}</span>
            </div>
          </div>
        ))}
      </div>
